...
payload = { 'mkey': config.CERT_NAME + '_' + VERSION, 'vdom': 'global', 'type': 'CertKey' }
...
def getcertificategroupdetails():
    response = r.get(config.ADC_URL + '/api/system_certificate_local_cert_group_child_group_member?pkey=' + config.CERT_NAME ) 
...
response = r.post(config.ADC_URL + '/api/system_certificate_local_cert_group_child_group_member?pkey=' + config.CERT_NAME, json = payload) 
...
for i in group_details:
    if i['default'] == 'enable':
        ...
        response = r.put(config.ADC_URL + '/api/system_certificate_local_cert_group_child_group_member?pkey=' + config.CERT_NAME + '&mkey=' + i['mkey'], json = payload) 
        if response.status_code == requests.codes.ok:
            pass
        else:
...
for i in group_details:
    if i['default'] == 'disable':
        ...
        response = r.delete(config.ADC_URL + '/api/system_certificate_local_cert_group_child_group_member?pkey=' + config.CERT_NAME + '&mkey=' + i['mkey']) 
        if response.status_code == requests.codes.ok:
            response = r.delete(config.ADC_URL + '/api/system_certificate_local?mkey=' + i['local_cert']) 
        else:
